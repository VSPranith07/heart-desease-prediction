...
# ── Auto-train if model files don't exist (for Streamlit Cloud) ───────────────
def train_and_save():
    import os
    ...
    ca       = np.random.choice([0,1,2,3,4],n,p=[0.58,0.22,0.12,0.07,0.01])
    thal     = np.random.choice([0,1,2,3],n,p=[0.01,0.06,0.55,0.38])
    risk_score = ((age>55).astype(int)*1.2+(cp<2).astype(int)*1.5+
                  (thalach<140).astype(int)*1.0+(exang==1).astype(int)*1.3+
                  (oldpeak>1.5).astype(int)*1.1+(ca>0).astype(int)*1.4+
                  (thal==3).astype(int)*1.2+np.random.normal(0,0.5,n))
    ...
    X = df_fe[feature_cols].values; y = df_fe['target'].values
    ...
    roc_auc   = auc(fpr,tpr)
    cv_scores = cross_val_score(best_model,X_train_s,y_train,cv=cv,scoring='accuracy')
    ...
    metrics = {
        'test_accuracy':round(acc,4),'cv_accuracy':round(float(grid.best_score_),4),
        'cv_std':round(float(cv_scores.std()),4),'roc_auc':round(roc_auc,4),
        'best_params':grid.best_params_,'confusion_matrix':cm.tolist(),
        'classification_report':cr,'fpr':fpr.tolist(),'tpr':tpr.tolist(),
        'cv_scores':cv_scores.tolist()
    }
    with open('models/metrics.json','w') as f:
        json.dump(metrics, f, indent=2)

# ── Load model artefacts (auto-trains if missing) ─────────────────────────────
@st.cache_resource
def load_artefacts():
    import os
    if not os.path.exists("models/svm_model.pkl"):
        with st.spinner("⚙️  First launch: training RBF-SVM model… (~30 sec)"):
            train_and_save()
